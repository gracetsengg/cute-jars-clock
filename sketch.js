...
function draw() {
  ...
  noStroke();
  textSize(28);
  text(`${h}:${nf(m,2)}:${nf(s,2)} ${ampm}`, width / 2, height - 40);
}
...
function updateAndDraw(arr, col) {
  for(let b of arr) b.update(), b.display(col);
}
...
class Bouncer {
  ...
  display(col) {
    ...
    if(this.symbol==="X"){
      ...
      ellipse(0,0,14,14);
      fill(255,180); ellipse(-3,-3,4,4);
    } else {
      noStroke(); fill(col); ellipse(0,0,5,5);
      fill(255,220); ellipse(random(-2,2),random(-2,2),2,2);
    }
    pop();
  }
}
